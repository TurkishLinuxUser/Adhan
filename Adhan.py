...
app_dir = f"{home_dir}/.local/share/adhan"


pid_dir = f"{app_dir}/"
pid_file = os.path.join(pid_dir, "adhan.pid")
...
filename = f"{app_dir}/settings.json"
...
class Log:
    def __init__(self, logfilename=f'{app_dir}/adhan.log'):
...
class EzanProgrami:
    def __init__(self, root):
        ...
        home_dir = os.path.expanduser("~")
        self.settings_file = f"{app_dir}/settings.json"

        
        
        home_dir = os.path.expanduser("~")
        self.image = Image.open(f"{app_dir}/icons/icon512x512.png")
        ...
        photo = tk.PhotoImage(file=f"{app_dir}/icons/sound.png")  
    ...
    def delete_azan(self, event):
        ...
        delete_azan_path = f"{azan_dir}{selected_azan}.mp3"
    ...
    def play_selected_azan(self, event):
        selected_azan = self.azan_var.get()
        sound_file = f"{app_dir}/azan/{selected_azan}.mp3"  
    ...
    def create_widgets(self):
        ...
        ttk.Label(main_frame, text=lang_manager.get_text("program_title"), font=(f"{app_dir}/{pfont}", 16)).grid(row=0, column=0, columnspan=2, pady=10)
        for i, prayer_name in enumerate(self.prayer_names):
            ttk.Label(main_frame, text=lang_manager.get_text(prayer_name) + ":", font=(f"{app_dir}/{pfont}", 12), anchor="w").grid(row=i+1, column=0, padx=10, pady=5, sticky="w")
            ttk.Label(main_frame, textvariable=self.prayer_times[prayer_name], font=(f"{app_dir}/{pfont}", 12), anchor="e").grid(row=i+1, column=1, padx=10, pady=5, sticky="e")
    ...
    def create_settings_widgets(self):
        ...
        ttk.Label(self.settings_window, text=lang_manager.get_text("language") + ":", font=(f"{app_dir}/{pfont}", 12)).grid(row=1, column=0, padx=10, pady=5, sticky="w")
        self.language_dropdown = ttk.Combobox(self.settings_window, textvariable=self.language_var, values=list(lang_manager.languages.keys()))
        self.language_dropdown.grid(row=1, column=1, padx=10, pady=5, sticky="ew")

        ttk.Label(self.settings_window, font=(f"{app_dir}/{pfont}", 12), text=lang_manager.get_text("city") + ":").grid(row=2, column=0, padx=10, pady=5, sticky="w")
        self.city_entry = tk.Entry(self.settings_window, textvariable=self.city_var)
        self.city_entry.grid(row=2, column=1, padx=10, pady=5, sticky="ew")

        ttk.Label(self.settings_window, font=(f"{app_dir}/{pfont}", 12), text=lang_manager.get_text("country") + ":").grid(row=3, column=0, padx=10, pady=5, sticky="w")
        self.country_entry = tk.Entry(self.settings_window, textvariable=self.country_var)
        self.country_entry.grid(row=3, column=1, padx=10, pady=5, sticky="ew")

        ttk.Label(self.settings_window, font=(f"{app_dir}/{pfont}", 12), text=lang_manager.get_text("select_azan") + ":").grid(row=0, column=0, padx=10, pady=5, sticky="w")
        ...
        arti_photo = tk.PhotoImage(file=f"{app_dir}/icons/arti.png")
        ...
        trash_photo = tk.PhotoImage(file=f"{app_dir}/icons/trash.png")
    ...
    def save_settings_and_close(self):
        ...
        if current_settings and (current_settings["city"] != city or current_settings["country"] != country):
            file_name = f"{app_dir}/prayer_times_{datetime.now().strftime('%Y-%m-%d')}.json"
    ...
    def play_ezan_sound(self):
        ...
        adhan = f"{app_dir}/azan/{azanval}.mp3"  
